...
class ResNetBackbone(nn.Module):
    ...
    def __init__(self, layers, dcn_layers=[0, 0, 0, 0], dcn_interval=1, atrous_layers=[], block=Bottleneck, norm_layer=nn.BatchNorm2d):
        ...
        self.layer1_attention = ChannelAttention(512)
        self.layer2_attention = ChannelAttention(1024)
        self.layer3_attention = ChannelAttention(2048)
        self.layer4_attention = ChannelAttention(4096)
    ...
    def forward(self, x):
        ...
        for i, layer in enumerate(self.layers):
            ...
            if i == 1:
                ...
                x = self.layer1_attention(x)
            elif i == 20:
            

                x = self.layer2_attention(x)
            elif i == 30:
         
                x = self.layer3_attention(x)
            elif i == 40:
       

                x = self.layer4_attention(x)
