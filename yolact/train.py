...
class NetLoss(nn.Module):
    ...
    def forward(self, images, targets, masks, num_crowds):
        ...
        if self.contexual_flag == 1:
            backbone_outs = self.yolact.backbone(images)
            layer3_feature_map = backbone_outs[2]
            layer4_feature_map = backbone_outs[3]
            # Compute the contextual loss using the extracted feature maps
            contextual_loss = self.contextual_loss_fn(layer3_feature_map, layer4_feature_map)

            # Add the contextual loss to the losses dictionary
            losses.update({'contextual_loss': contextual_loss})
...
def train():
    ...
    contextual_loss_fn = ContextualLoss()
